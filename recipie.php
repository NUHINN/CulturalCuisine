...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            ...
            $stmt = $db->prepare("INSERT INTO Recipes (Name, Description, Region, CuisineType) VALUES (:name, :description, :region, :ctype)");
            $ok = $stmt->execute([
                ':name' => $name,
            ...
            echo $ok ? "Recipe added successfully!" : "Error: Could not add the recipe.";
        } elseif ($action === 'edit') {
            ...
            $stmt = $db->prepare("UPDATE Recipes SET Name = :name, Description = :description, Region = :region, CuisineType = :ctype WHERE RecipeID = :id");
            $ok = $stmt->execute([
                ':name' => $name,
                ':description' => $description,
                ':region' => $region,
            ...
            echo $ok ? "Recipe updated successfully!" : "Error: Could not update the recipe.";
        } elseif ($action === 'delete') {
            $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;

            $stmt = $db->prepare("DELETE FROM Recipes WHERE RecipeID = :id");
            $ok = $stmt->execute([':id' => $id]);
            echo $ok ? "Recipe deleted successfully!" : "Error: Could not delete the recipe.";
        }
    }
}
